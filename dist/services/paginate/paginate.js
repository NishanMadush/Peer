import { MAX_PAGE_SIZE } from '../../constants/helper';
const paginate = (schema) => {
     ...
     * @param {number} [options.limit] - Maximum number of results per page (default = MAX_PAGE_SIZE). Otherwise default can be 10
    ...
    schema.static('paginate', async function (filter, options) {
        ...
        const limit = options.limit && parseInt(options.limit.toString(), MAX_PAGE_SIZE) > 0 ? parseInt(options.limit.toString(), MAX_PAGE_SIZE) : MAX_PAGE_SIZE;
        const page = options.page && parseInt(options.page.toString(), MAX_PAGE_SIZE) > 0 ? parseInt(options.page.toString(), MAX_PAGE_SIZE) : 1;
