...
import setupTestDB from '../../services/jest/setupTestDB';
...
describe('Auth routes', () => {
    describe(`POST ${API_ENDPOINT}/auth/register`, () => {
        let newUser;
        beforeEach(() => {
            newUser = {
                username: 'Username_1',
        ...
        test('should return 201 and successfully register user if request data is ok', async () => {
            const res = await request(app).post(`${API_ENDPOINT}/auth/register`).send(newUser).expect(httpStatus.CREATED);
        ...
        test('should return 400 error if email is invalid', async () => {
            ...
            await request(app).post(`${API_ENDPOINT}/auth/register`).send(newUser).expect(httpStatus.BAD_REQUEST);
        });
    ...
    describe(`POST ${API_ENDPOINT}/auth/login`, () => {
        test('should return 200 and login user if email and password match', async () => {
            ...
            const res = await request(app).post(`${API_ENDPOINT}/auth/login`).send(loginCredentials).expect(httpStatus.OK);
            expect(res.body.user).toEqual({
                id: expect.anything(),
    ...
    describe(`POST ${API_ENDPOINT}/auth/logout`, () => {
        test('should return 204 if refresh token is valid', async () => {
            await insertUsers([userOne]);
            const expires = moment().add(config.jwt.refreshExpirationDays, 'days');
            const refreshToken = tokenService.generateToken(userOne._id, expires, tokenTypes.REFRESH);
            await tokenService.saveToken(refreshToken, userOne._id, expires, tokenTypes.REFRESH);
            await request(app).post(`${API_ENDPOINT}/auth/logout`).send({ refreshToken }).expect(httpStatus.NO_CONTENT);
            const dbRefreshTokenDoc = await Token.findOne({ token: refreshToken });
            expect(dbRefreshTokenDoc).toBe(null);
        });
        test('should return 400 error if refresh token is missing from request body', async () => {
            await request(app).post(`${API_ENDPOINT}/auth/logout`).send().expect(httpStatus.BAD_REQUEST);
        });
        test('should return 404 error if refresh token is not found in the database', async () => {
            await insertUsers([userOne]);
            const expires = moment().add(config.jwt.refreshExpirationDays, 'days');
            const refreshToken = tokenService.generateToken(userOne._id, expires, tokenTypes.REFRESH);
            await request(app).post(`${API_ENDPOINT}/auth/logout`).send({ refreshToken }).expect(httpStatus.NOT_FOUND);
        });
    ...
    describe(`POST ${API_ENDPOINT}/auth/refresh-tokens`, () => {
        test('should return 200 and new auth tokens if refresh token is valid', async () => {
            await insertUsers([userOne]);
            const expires = moment().add(config.jwt.refreshExpirationDays, 'days');
            const refreshToken = tokenService.generateToken(userOne._id, expires, tokenTypes.REFRESH);
            await tokenService.saveToken(refreshToken, userOne._id, expires, tokenTypes.REFRESH);
            const res = await request(app).post(`${API_ENDPOINT}/auth/refresh-tokens`).send({ refreshToken }).expect(httpStatus.OK);
            expect(res.body.user).toEqual({
                id: expect.anything(),
                firstName: userOne.firstName,
                lastName: userOne.lastName,
        ...
        test('should return 400 error if refresh token is missing from request body', async () => {
            await request(app).post(`${API_ENDPOINT}/auth/refresh-tokens`).send().expect(httpStatus.BAD_REQUEST);
        });
        test('should return 401 error if refresh token is signed using an invalid secret', async () => {
            ...
            await tokenService.saveToken(refreshToken, userOne._id, expires, tokenTypes.REFRESH);
            await request(app).post(`${API_ENDPOINT}/auth/refresh-tokens`).send({ refreshToken }).expect(httpStatus.UNAUTHORIZED);
        });
    ...
    describe(`POST ${API_ENDPOINT}/auth/reset-password`, () => {
        test('should return 204 and reset the password', async () => {
            ...
            await request(app)
                .post(`${API_ENDPOINT}/auth/reset-password`)
    ...
    describe(`POST ${API_ENDPOINT}/auth/verify-email`, () => {
        test('should return 204 and verify the email', async () => {
            ...
            await request(app)
                .post(`${API_ENDPOINT}/auth/verify-email`)
        ...
        test('should return 400 if verify email token is missing', async () => {
            await insertUsers([userOne]);
            await request(app).post(`${API_ENDPOINT}/auth/verify-email`).send().expect(httpStatus.BAD_REQUEST);
        });
